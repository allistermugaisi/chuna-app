...
const initialState = {
  ...
  // Full statement
  fullStatement: null, // base64 image string from base_report
  loadingFull: false,
...
export const fetchFullStatement = createAsyncThunk(
  ...
  async ({ date_from, date_to, doc_no = "" }, { rejectWithValue }) => {
    try {
      ...
      // base_report is a base64 image string
      return json?.data?.base_report ?? null;
    } catch (e) {
      return rejectWithValue({ message: e?.message ?? "Network error." });
    }
  },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchFullStatement.fulfilled, (state, action) => {
        ...
        state.fullStatement = action.payload; // base64 string or null
      })
