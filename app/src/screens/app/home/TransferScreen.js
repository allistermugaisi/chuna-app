...
export default function TransferScreen({ navigation, route }) {
  ...
  const [extDestAccount, setExtDestAccount] = useState(null); // chosen from validated member's accounts
  ...
  useEffect(() => {
    setDestAccount(null);
    setMemberNo("");
    setExtDestAccount(null);
    dispatch(clearValidatedMember());
  }, [transferType]);
  ...
  const onSubmit = useCallback(
    async (data) => {
      ...
      if (transferType === "fosa_to_bosa") {
        ...
        // External — destination is a validated member's chosen FOSA account
        if (!validatedMember) {
          Alert.alert("Missing", "Please validate the member number first.");
          return;
        }
        if (!extDestAccount) {
          Alert.alert("Missing", "Please select the destination account.");
          return;
        }
        await dispatch(
          transferFosa({
            source_account_no: sourceAccount.account_no,
            destination_account: extDestAccount.account_no,
            amount,
            doc_no,
            app_type: "",
          }),
      ...
      <View style={[s.container, { paddingTop: insets.top }]}>
        ...
        >
          ...
          {transferType === "fosa_external" ? (
            // External — validate member number first, then pick their account
            <View style={s.extWrap}>
              {/* Member number input + Validate button */}
              <View style={s.memberInputRow}>
                <View
                  style={[
                    s.memberInputWrap,
                    validatedMember && s.memberInputValid,
                  ]}
                >
                  <MaterialCommunityIcons
                    name="account-search-outline"
                    size={18}
                    color={validatedMember ? GREEN : TEXT_FAINT}
                  />
                  <TextInput
                    style={s.memberInput}
                ...
                <TouchableOpacity
                  style={[
                    s.validateBtn,
                    (!memberNo.trim() || validating) && s.validateBtnDisabled,
                  ]}
                  onPress={() => {
                    if (!memberNo.trim()) return;
                    setExtDestAccount(null);
                    dispatch(validateMember({ member_no: memberNo.trim() }));
                  }}
                  disabled={!memberNo.trim() || validating}
                >
                  {validating ? (
                    <ActivityIndicator size="small" color="#fff" />
                  ) : (
                    <Text style={s.validateBtnTxt}>Verify</Text>
                  )}
                </TouchableOpacity>
              </View>

              {/* Validated member card */}
              {validatedMember && (
                <View style={s.memberCard}>
                  <View style={s.memberCardTop}>
                    <View style={s.memberAvatar}>
                      <Text style={s.memberAvatarTxt}>
                        {validatedMember.member_name
                          .trim()
                          .split(" ")
                          .map((w) => w[0])
                          .slice(0, 2)
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={s.memberName}>
                        {validatedMember.member_name.trim()}
                      </Text>
                      <Text style={s.memberMeta}>
                        Member {validatedMember.member_no} ·{" "}
                        {validatedMember.mobile_no}
                      </Text>
                    </View>
                  ...
                  <Text style={s.memberAccLabel}>
                    Select destination account
                  </Text>
                  {validatedMember.fosa_accounts.map((acc) => {
                    const isChosen =
                      extDestAccount?.account_no === acc.account_no;
                    ...
                    return (
                      <TouchableOpacity
                        ...
                        style={[
                          s.extAccItem,
                          isChosen && s.extAccItemChosen,
                          !isActive && s.extAccItemDisabled,
                        ]}
                        onPress={() => isActive && setExtDestAccount(acc)}
                        activeOpacity={isActive ? 0.8 : 1}
                      >
                        <View
                          style={[
                            s.extAccIcon,
                            isChosen && { backgroundColor: GREEN },
                          ]}
                        >
                          <MaterialCommunityIcons
                            name="bank-outline"
                            size={16}
                            color={isChosen ? "#fff" : GREEN}
                          />
                        </View>
                        <View style={{ flex: 1 }}>
                          <Text
                            style={[s.extAccName, isChosen && { color: GREEN }]}
                          >
                            {acc.account_name.trim()}
                          </Text>
                          <Text style={s.extAccNo}>{acc.account_no}</Text>
                          <Text style={s.extAccBal}>KES {acc.balance}</Text>
                        </View>
                        {isChosen ? (
                          <Ionicons
            ...
            (destAccount || extDestAccount) && (
              <View style={s.summaryCard}>
                ...
                {extDestAccount && (
                  <SummaryRow
                    label="Account no."
                    value={extDestAccount.account_no}
                  />
                )}
                <SummaryRow
                  label="Amount"
        ...
        <View style={[s.footer, { paddingBottom: insets.bottom + 12 }]}>
          <TouchableOpacity
            style={[
              ...
              (transferring ||
                !sourceAccount ||
                (transferType === "fosa_external" && !extDestAccount)) &&
...
const s = StyleSheet.create({
  ...
  memberInputValid: { borderColor: GREEN },
  memberInput: {
    flex: 1,
    fontSize: 14,
    fontWeight: "600",
  ...
  validateBtnTxt: { color: "#fff", fontWeight: "700", fontSize: 14 },
  ...
  memberAvatarTxt: { color: "#fff", fontWeight: "800", fontSize: 15 },
  memberName: { fontSize: 14, fontWeight: "700", color: TEXT_DARK },
  memberMeta: { fontSize: 11, color: TEXT_FAINT, marginTop: 2 },
  verifiedBadge: {
    flexDirection: "row",
    alignItems: "center",
    gap: 4,
  ...
  verifiedTxt: { fontSize: 10, fontWeight: "700", color: GREEN },
  memberAccLabel: {
    fontSize: 11,
    fontWeight: "700",
    color: TEXT_FAINT,
  ...
  extAccItemChosen: { borderColor: GREEN, backgroundColor: "#F0FBF4" },
  extAccItemDisabled: { opacity: 0.45 },
  extAccIcon: {
    width: 36,
    height: 36,
    borderRadius: 10,
  ...
  extAccNo: { fontSize: 11, color: TEXT_FAINT },
  extAccBal: { fontSize: 12, color: GREEN, fontWeight: "600", marginTop: 2 },
  extAccRadio: {
    width: 20,
    height: 20,
    borderRadius: 10,
  ...
  // External input (keep for fallback)
  extInputWrap: {
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
