        ...
        {loading ? (
          ...
          <View style={am.loadingWrap}>
            <MaterialCommunityIcons
              ...
              color={TEXT_FAINT}
            />
            <Text style={am.emptyTxt}>No withdrawable accounts found</Text>
          </View>
        ) : (
          <FlatList
...
export default function WithdrawScreen({ navigation, route }) {
  ...
  // Fetch withdrawable accounts on mount
  useEffect(() => {
    dispatch(
      fetchWithdrawableAccounts({
    ...
    async (data) => {
      ...
      await dispatch(
        initiateWithdraw({
          ...
          doc_no: `MPE-${Date.now().toString().slice(-6)}`,
  ...
  const renderStep1 = () => (
    <>
      ...
      {/* Account selector */}
      <TouchableOpacity
        style={[s.accountRow, !selectedAccount && s.accountRowEmpty]}
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          {selectedAccount ? (
            <>
              <Text style={s.accountValue}>{selectedAccount.account_name}</Text>
              <Text style={s.accountNo}>{selectedAccount.account_no}</Text>
            </>
          ) : (
            <Text style={s.accountPlaceholder}>
              {loadingAccounts ? "Loading accounts…" : "Tap to select account"}
            </Text>
          )}
        </View>
        {loadingAccounts ? (
  ...
  return (
    ...
    >
      <View style={[s.container]}>
        ...
        {/* Account select modal — rendered inside KeyboardAvoidingView so it
            sits above the keyboard on iOS */}
        <AccountSelectModal
...
const s = StyleSheet.create({
  ...
  accountRowEmpty: { borderColor: "#FCA5A5", borderStyle: "dashed" },
  accountIconWrap: {
    width: 38,
    height: 38,
    borderRadius: 10,
  ...
  accountNo: { fontSize: 11, color: TEXT_FAINT, marginTop: 2 },
  accountPlaceholder: { fontSize: 14, color: TEXT_FAINT, fontStyle: "italic" },
