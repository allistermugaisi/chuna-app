...
export default function FullStatementScreen({ navigation }) {
  ...
  const handleShare = useCallback(async () => {
    try {
      const file = await saveStatementFile();
      if (!file) return;
      ...
      if (await Sharing.isAvailableAsync()) {
        ...
        await Share.share({ message: `Statement saved to: ${file.path}` });
      }
    } catch (e) {
      console.log("Share failed:", e);
    }
  ...
  const saveStatementFile = useCallback(async () => {
  ...
  const savePdfWithBlobUtil = async (rawBase64) => {
    const cleanBase64 = rawBase64.replace(/^data:application\/pdf;base64,/, "");

    const filePath = `${ReactNativeBlobUtil.fs.dirs.CacheDir}/chuna-statement.pdf`;
  ...
  const handleViewPdf = async () => {
    try {
      ...
      console.log("View PDF failed:", e);
    }
  };
  // const handleViewPdf = async () => {
  //   try {
  //     const uri = await savePdfWithBlobUtil(fullStatement);
  ...
  //     setPdfOpen(true);
  //   } catch (error) {
  //     console.log("View PDF failed:", error);
  //   }
  // };
    ...
    <View style={[s.container, { paddingTop: insets.top }]}>
      ...
      {!loadingFull && !!fullStatement && (
        <View style={[s.bottomBar, { paddingBottom: insets.bottom + 12 }]}>
          ...
          <TouchableOpacity style={s.viewPdfBtn} onPress={handleViewPdf}>
            <MaterialCommunityIcons
              name="file-pdf-box"
              size={20}
              color="#fff"
            />
            <Text style={s.viewPdfTxt}>Open PDF</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={s.doneBtn}
      ...
      <Modal visible={pdfOpen} animationType="slide">
